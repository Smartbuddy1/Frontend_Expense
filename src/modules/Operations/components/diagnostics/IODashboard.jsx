...
const IODashboard = ({ machineId, status, machineDetails }) => {
    ...
    const drawPDF = (clientImgObj, sbImgObj) => {
        ...
        autoTable(doc, {
          ...
          didDrawPage: (data) => {
            ...
            if (sbImgObj && sbImgObj.base64) {
              try {
                doc.addImage(sbImgObj.base64, sbImgObj.ext, 14, sepY + 2.5, 15, 5);
              } catch (err) {}
            }

            doc.setFontSize(8);
            doc.setTextColor(100, 100, 100);
            doc.text('AARYA INNOVTECH PVT. LTD. CIN: U29305MH2019PTC327551 | +91 9359604384 | https://aaryainnovtech.com/', sbImgObj?.base64 ? 32 : 14, sepY + 5);
            doc.text('Nashik Office: Flat No.4A, Sayali Darshan A-Wing, Makhamalabad Road, Nashik-422003.', sbImgObj?.base64 ? 32 : 14, sepY + 9);
    ...
    const generatePDFReport = async () => {
        ...
        if (!clientImgObj) {
          const defaultLeftUrl = window.location.origin + `/aarya_logo.jpeg`;
